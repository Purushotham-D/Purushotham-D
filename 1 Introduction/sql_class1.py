...
# establishing connection creating a database name tutorial.db
conn = sqlite3.connect('tutorial_1.db')


# an ability to delete and add something
c = conn.cursor()
...
def data_entry_2(ind,date, value):
        c = conn.cursor()
        query = 'INSERT INTO stuffToPlot VALUES({}, "{}",{})'.format(ind,date, value)
        c.execute(query)
        conn.commit() # saving 
        c.close() # clasing file 
